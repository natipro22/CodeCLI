...
[Command("generate(g) mediatr(m) query(q)", @"^\b(generate|g)\b \b(mediatr|m)\b \b(query|q)\b$", Description = "Creates a new, generic request query definition using MediatR in the given project.")]
